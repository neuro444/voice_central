...
import { SESSION_COOKIE, verifyAndDecodeSessionToken, verifySessionToken } from "@/lib/auth";
...
const ALLOWED_ROUTES = [
  ...
  /^orders\/recent$/,
  /^api\/approvals$/,
  /^api\/approvals\/[A-Za-z0-9_-]+\/(?:approve|reject)$/,
  /^menu$/,
  /^crm\/customers$/,
];
...
export async function POST(
  ...
  const apiKey = process.env.CHAT_MANAGER_API_KEY;
  if (!baseUrl || !apiKey) {
    return NextResponse.json({ error: "Chat Manager integration is not configured" }, { status: 503 });
  }
